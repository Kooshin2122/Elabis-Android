...
const OTP_Screen = ({ route }) => {
    const { navigate, } = useNavigation()
    const { userInfo } = route?.params ?? { otherParam: null };
    ...
    // Main Function
    const SaveTheUser = (values) => {
        // Show OTP Object
        console.log('----------OTP', values);
        // Show User Info 
        console.log('user info ----------', userInfo);
        // 1) Check the OTP if it's correct or not then -->
        // 2) Save the user to the data base () then -->
        // 3) Store user info in to the Local storage then --> 
        // 4) Navigate Home screen after saving the user -->
    }
    //
    const boxTwo = useRef()
                ...
                <ScrollView style={styles.scrollCon}>
                    ...
                    <View style={styles.formCon}>
                        ...
                        <Text style={styles.description}>
                            We have sent you an OTP code to this phone number ( {userInfo?.phoneNumber ?? null} ). Please fill  the boxes correctly
                        </Text>
                        <Devider />
                        <Formik
                            ...
                            {({ handleChange, handleBlur, handleSubmit, values, errors }) => {
                                return (
                                    <View>
                                        ...
                                        <Devider />
                                        <CustomButton title="DONE" clickHandler={handleSubmit} />
                                    </View>
                                )
                            }}
                        </Formik>
...
const OTP_FEILD = ({ value, name, reference, returnKeyType = "next", defaultFocus, onChangeText = () => { }, onSubmitEditing = () => { } }) => {
    return (
